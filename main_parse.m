...
%% Extracting data
pathToData =fullfile(pwd,'/dumpedData/');
bowing = false;

if bowing
    pathToData =fullfile(pathToData,'/bowing/');
else
    pathToData =fullfile(pathToData,'/squat/');
end

% ----Human forces
 filename_dumpForces = fullfile(pathToData,'humanForces/data.log');
 forces = parseHumanForces(filename_dumpForces, 4);
% % since this dataset consider 4 forces exchanged with the shoes and the
% % robot.

% ----Human state
filename_dumpState = fullfile(pathToData,'humanState/data.log');
state = parseHumanState(filename_dumpState);

% ----Dynamic estimation (vector d)
filename_dumpDynEst = fullfile(pathToData,'humanDynEstimation/data.log');
...
for j = 1: size(forces.forces.RightFoot.value,1)
    ...
    interpData.forces.RightHand.value(j,:) = interp1(forces.time, forces.forces.RightHand.value(j,:), sampling_master_time);
    interpData.forces.LeftHand.value(j,:)  = interp1(forces.time, forces.forces.LeftHand.value(j,:), sampling_master_time);
end
